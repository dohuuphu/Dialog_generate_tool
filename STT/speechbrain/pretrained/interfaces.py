...
class EncoderASR(Pretrained):
    ...
    def build_lm(self):
        ...
        vocab_list[self.tokenizer.word_delimiter_token_id] = " "
        ...
        decoder = build_ctcdecoder(vocab_list, lm_path)
        return decoder
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.tokenizer = self.hparams.tokenizer
        ...
        with torch.no_grad():
            out = self.mods.encoder(x)
            self.vocab_size = out.shape[-1] 
        self.decoder = self.build_lm()
    ...
    def transcribe_batch(self, wavs, wav_lens):
        ...
        with torch.no_grad():
            wav_lens = wav_lens.to(self.device)
            self.vocab_size = wav_lens - 1
            encoder_out = self.encode_batch(wavs, wav_lens)
            pred_str = [self.decoder.decode(
              logit.detach().cpu().numpy(), beam_width=100) for logit in encoder_out]
